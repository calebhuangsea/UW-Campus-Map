...
public class MarvelParser {
     ...
     * @return return a list contains two Map, one has characters as keys and values as each characters' book
     *            and one has books as keys and values as characters appear in that book
     */
    // TODO: Replace 'void' with the type you want the parser to produce
    public static List<Map<String, List<String>>> parseData(String filename) {
        List<String> lines = readLines(filename);
        List<Map<String, List<String>>> list = new ArrayList<>();
        Map<String, List<String>> character = new HashMap<>();
        ...
        for (String line : lines) {
            ...
            if (!character.containsKey(split[0])) {
                character.put(split[0], new ArrayList<>());
            }
            character.get(split[0]).add(split[1]);

            if (!books.containsKey(split[1])) {
                books.put(split[1], new ArrayList<>());
            }
            books.get(split[1]).add(split[0]);
